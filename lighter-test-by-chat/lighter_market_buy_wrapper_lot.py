#!/usr/bin/env python3
# Lighter MAINNET — BTC MARKET BUY (lot-bazlı, doğru ölçek)
...
TARGET_NOTIONAL    = float(os.getenv("TARGET_NOTIONAL_USDC", "10.0"))        # hedef notional (USD)
ACCOUNT_LEVERAGE   = float(os.getenv("ACCOUNT_LEVERAGE", "10.0"))           # log amaçlı
PRICE_HINT_USD     = float(os.getenv("PRICE_HINT_USD", "110000"))           # fiyat ipucu (sizing)
...
async def main():
    ...
    log.info("=" * 58)
    log.info("🚀 Lighter MAINNET — BTC MARKET BUY (lot-bazlı @ IOC)")
    log.info("=" * 58)
    ...
    try:
        ...
        # --- 1) Market metadata: base_lot_size & (opsiyonel) price_tick_size
        det = await ord_api.order_book_details(market_id=MARKET_ID)
        # build'lara göre alan isimleri değişebiliyor:
        base_lot = (
            getattr(det, "base_lot_size", None) or
            getattr(det, "base_step", None) or
            getattr(det, "baseLotSize", None) or
            0.000001  # fallback
        )
        base_lot = float(base_lot)

        price_tick = (
            getattr(det, "price_tick_size", None) or
            getattr(det, "price_step", None) or
            getattr(det, "priceTickSize", None) or
            0.01  # 1 cent fallback
        )
        ...
        # --- 2) Sizing: hedef notional → BTC → lot adedi
        base_qty_btc  = TARGET_NOTIONAL / PRICE_HINT_USD
        base_qty_btc *= SAFETY_RATIO  # küçük buffer
        if base_qty_btc <= 0:
            log.error("Hesaplanan BTC miktarı <= 0; PRICE_HINT_USD veya TARGET_NOTIONAL'ı artır.")
            sys.exit(1)

        lots = int(base_qty_btc / base_lot)  # DAİMA lot'a böl
        if lots < 1:
            lots = 1  # en az 1 lot

        est_notional = lots * base_lot * PRICE_HINT_USD
        est_margin   = est_notional / max(ACCOUNT_LEVERAGE, 1.0)

        log.info("🧮 Sizing (LOT bazlı)")
        log.info(f"   Lots          : {lots} lot  (lot={base_lot:.8f} BTC)")
        ...
        # --- 3) MARKET BUY gönderimi (IOC) — wrapper
        ...
        # --- 4) Kısa bekleme + read-only kontroller
        await asyncio.sleep(2)

        # account_txs bazı build'larda auth gerektirir → dene, olmazsa uyarmakla yetin.
        try:
            # not: bir çok sürüm 'by/value' ister
