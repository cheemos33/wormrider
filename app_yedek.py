        ...
        if agg_snapshot:
                ...
                if instant_imbalance:
                    ...
                    signal = {
                        ...
                        'tp_price': current_price * (1 + config.INSTANT_XL_TP_PCT) if instant_imbalance["direction"] == "long" else current_price * (1 - config.INSTANT_XL_TP_PCT),
                        'sl_price': current_price * (1 - config.INSTANT_XL_SL_PCT) if instant_imbalance["direction"] == "long" else current_price * (1 + config.INSTANT_XL_SL_PCT),
        ...
        if instant_bids and instant_asks:
            ...
            if instant_imbalance:
                ...
                signal = {
                    ...
                    'direction': instant_imbalance['direction'],
                    'entry_price': current_price,
                    'tp_price': current_price * (1 + config.INSTANT_TP_PCT) if instant_imbalance["direction"] == "long" else current_price * (1 - config.INSTANT_TP_PCT),
                    'sl_price': current_price * (1 - config.INSTANT_SL_PCT) if instant_imbalance["direction"] == "long" else current_price * (1 + config.INSTANT_SL_PCT),
                    'bid_volume': instant_imbalance['bid_volume'],
                    'ask_volume': instant_imbalance['ask_volume'],
                    'imbalance_ratio': instant_imbalance['imbalance_ratio'],
                    'cvd_slope': 'N/A',
        ...
        if historical_imbalance and instant_imbalance:
            ...
            if (historical_imbalance['direction'] == instant_imbalance['direction'] and
                ...
                signal = {
                    ...
                    'entry_price': current_price,
                    'tp_price': current_price * (1 + config.INSTANT_XL_TP_PCT) if historical_imbalance["direction"] == "long" else current_price * (1 - config.INSTANT_XL_TP_PCT),
                    'sl_price': current_price * (1 - config.INSTANT_XL_SL_PCT) if historical_imbalance["direction"] == "long" else current_price * (1 + config.INSTANT_XL_SL_PCT),
        ...
        if not active_instant_sq and not pending_instant_sq:
            ...
            for recent_signal in recent_instant_signals:
                ...
                if (current_time_ms - recent_timestamp) < 300000:  # 5 minutes
                    ...
                    signal_dir = recent_signal["direction"]
                    
                    # Initialize or update
                    if last_direction is None:
                        last_direction = signal_dir
                        consecutive_count = 1
                    elif signal_dir == last_direction:
                        consecutive_count += 1
                    else:
                        ...
                        print(f"DEBUG: consecutive_count={consecutive_count}, instant_imbalance={instant_imbalance}")
