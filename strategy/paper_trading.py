...
class PaperTradingMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.running:
            try:
                # Get active signals for all 4 strategies
                ...
                if pending_instant_sq and self.current_price:
                    self._activate_pending_signal(pending_instant_sq)
    ...
    def _activate_pending_signal(self, signal: Dict[str, Any]):
        ...
        json_logger.json_logger.log_entry(signal)
        # Execute live trade if enabled
        try:
            live_trading.live_trading.execute_signal(signal)
        except Exception as e:
